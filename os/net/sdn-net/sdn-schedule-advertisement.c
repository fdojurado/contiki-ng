...
int sdn_sa_input(void)
{
    ...
    if (sdnip_htons(SDN_SA_SCHEDULES_BUF->seq) < sequence_number)
    {
        PRINTF("pkt already processed, dropping\n");
        return 0;
    }
    // Update sequence number received
    sequence_number = sdnip_htons(SDN_SA_SCHEDULES_BUF->seq) + 1;
    // Process schedules
    uint8_t num_schedules, i, type, channel_offset, time_offset;
    linkaddr_t scr, dst;
    num_schedules = SDN_SA_SCHEDULES_BUF->payload_len / SDN_SAPL_LEN;
    for (i = 0; i < num_schedules; i++)
    {
        // Only process SA for us
        scr.u16 = sdnip_htons(SDN_SA_SCHEDULES_PAYLOAD(i)->scr.u16);
        if (linkaddr_cmp(&scr, &linkaddr_node_addr))
        {
            type = SDN_SA_SCHEDULES_PAYLOAD(i)->type;
            channel_offset = SDN_SA_SCHEDULES_PAYLOAD(i)->channel_offset;
            time_offset = SDN_SA_SCHEDULES_PAYLOAD(i)->time_offset;
            dst.u16 = sdnip_htons(SDN_SA_SCHEDULES_PAYLOAD(i)->dst.u16);
